...
export function parseGetMicrocredentialQuizStudentAttemptListOutput(rawJson = {}, status = 200) {
    ...
    const quizAttemptList = rawList.map(item => ({
        ...
        correctQuestionsCount: item?.correctQuestionsCount ?? item?.CorrectQuestionsCount ?? item?.correctCount ?? item?.CorrectCount ?? 0,
        correctCount: item?.correctQuestionsCount ?? item?.CorrectQuestionsCount ?? item?.correctCount ?? item?.CorrectCount ?? 0,
        wrongAnswers: item?.wrongAnswers ?? item?.WrongAnswers ?? item?.wrongCount ?? item?.WrongCount ?? 0,
        wrongCount: item?.wrongAnswers ?? item?.WrongAnswers ?? item?.wrongCount ?? item?.WrongCount ?? 0,
        skippedQuestions: item?.skippedQuestions ?? item?.SkippedQuestions ?? item?.skippedCount ?? item?.SkippedCount ?? 0,
        skippedCount: item?.skippedQuestions ?? item?.SkippedQuestions ?? item?.skippedCount ?? item?.SkippedCount ?? 0,
